...
function collect() {
  ...
  const take = (keyPrefix, imagesetDir) => {
    ...
    entries.push({ key, name, group: keyPrefix, src: join(dir, svg) });
  };
...
for (const { key, name, group, src } of entries) {
  ...
  copyFileSync(src, join(destDir, `${name}.svg`));
}

const requirePath = (entry) => {
  const rel = relative(dirname(OUT_MAP), join(OUT_ASSETS, entry.group, `${entry.name}.svg`));
