...
function CardSlot({ chart, index, ids, width, height, session }: {
  ...
  const { dragging, target, armed, reducedMotion } = session;
  const { gesture, x, y, dismissed } = useCardDrag({ id: chart.id, index, ids, width, height, gap: GAP, session });
  ...
  return <Animated.View style={[{ flex: 1, height, minWidth: 0 }, slotStyle]}>
    <View pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={{ flex: 1, opacity: 0.3 }}>{face}</View>
    ...
    <GestureDetector gesture={gesture}>
      <Animated.View style={[{ position: 'absolute', inset: 0, shadowColor: '#000', shadowOffset: { width: 0, height: 5 }, shadowRadius: 10, elevation: 2 }, liftStyle]}>
        <Pressable accessibilityRole="radio" accessibilityLabel={`Step ${chart.name}, ring ${index + 1}`}
          ...
          accessibilityHint="Tap to choose the time stepper target. Drag to another card to reorder rings, or into the lower half of the screen to put away. Chart actions are below the row."
...
export function ActiveChartCards({ session }: { session: CardDragSession }) {
  ...
  useLayoutEffect(() => { invalidate(); }, [width, height, collapsed, invalidate]);
