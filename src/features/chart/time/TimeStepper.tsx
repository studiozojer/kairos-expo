...
export function IntervalDial({ unit, onChange, onTap, onReset, onScrollStart, offset, resetLabel = 'Return to now' }: {
  unit: number; onChange: (unit: number) => void; onTap: () => void; onReset: () => void; onScrollStart: () => void; offset?: string; resetLabel?: string;
}) {
  const theme = useTheme();
  const scroll = useRef<ScrollView>(null);
  const scrolling = useRef(false);
  const dragging = useRef(false);
  const [initialOffset] = useState(() => ({ x: 0, y: unit * DIAL_ROW_HEIGHT }));
  ...
  return <View style={{ flex: 1, minWidth: 0, height: 44 }}>
    ...
    <ScrollView ref={scroll} testID="time-interval-dial" showsVerticalScrollIndicator={false}
      style={{ flex: 1 }} contentOffset={initialOffset}
      ...
      accessibilityHint={`Swipe up or down to change the interval. Double-tap the stepper: ${resetLabel.toLowerCase()}.`}
...
export function TimeStepper() {
  ...
  const resetLabel = clock.kind === 'saved' ? 'Return to original time' : 'Return to now';
  const offset = clock.kind === 'saved' && clock.time === clock.origin ? 'Original' : timeOffset(clock.time, clock.origin);
  return <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 6, paddingVertical: 4,
    minHeight: 52, width: '100%' }}>
    <IntervalDial resetLabel={resetLabel} offset={offset} unit={clock.unit} onChange={unit => { lastTap.current = null; clock.selectUnit(unit); }} onTap={tap} onReset={reset}
    ...
    <StepButtons key={clock.targetId} interval={TIME_STEPS[clock.unit].label.toLowerCase()}
