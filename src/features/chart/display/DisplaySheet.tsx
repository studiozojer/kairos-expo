...
function DisplayEditor({ preset, presetName, bodyNames, config, onChangePreset: change, onSelectPreset, onClose, previewPosition, onChangePreviewPosition: setPreviewPosition }: DisplayEditorProps) {
    ...
    {aspects.enabledTypes.includes(a.displayName) && <NumberRow label={`${a.displayName} orb ±`} value={aspects.orbs.orbs[a.displayName] ?? a.defaultOrb} onChange={v => aspect({ orbs: { orbs: { ...aspects.orbs.orbs, [a.displayName]: v } } })}/>}
        ...
        content = <><Section title="Fill styling">
        <NumberRow label="Base opacity" value={preset.aspectOverlay.patternOpacity * 100} onChange={v => line({ patternOpacity: v / 100 })} max={100} step={1} suffix="%"/>
        <NumberRow label="Orb weighting" value={preset.aspectOverlay.patternOrbWeighting * 100} onChange={v => line({ patternOrbWeighting: v / 100 })} max={100} step={5} suffix="%"/>
        <Note>Base opacity is the fill of an exact pattern. Orb weighting fades patterns using their widest defining aspect: 0% is uniform; 100% fades fully at the tolerance limit.</Note>
        </Section><Section title="Tolerance"><NumberRow label="Pattern orb ±" value={patterns.orb} onChange={v => aspect({ patterns: { ...patterns, orb: v } })}/><Note>Every defining aspect must be within this tolerance.</Note></Section><Section title="Shapes">{PATTERN_NAMES.map(name => <Toggle key={name} label={name} leading={<PatternIcon name={name} />} value={patterns.enabledTypes.includes(name)} onChange={v => aspect({ patterns: { ...patterns, enabledTypes: v ? [...patterns.enabledTypes, name] : patterns.enabledTypes.filter(n => n !== name) } })}/>)}</Section></>;
    ...
    <NumberRow label="Orb weighting" value={preset.aspectOverlay.orbWeighting * 100} onChange={v => line({ orbWeighting: v / 100 })} max={300} step={5} suffix="%"/>
    <Note>Base thickness is the width of an exact aspect. Orb weighting makes wider aspects thinner: 0% keeps every line uniform; 100% uses a squared fade; up to 300% makes the fade steeper. A small minimum keeps lines visible.</Note>
